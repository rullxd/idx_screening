...
function OrderbookRows({ rows, isBid }: { rows: OrderbookRow[]; isBid: boolean }) {
 ...
 return Math.max(...rows.map((r) => parseFloat(r.volume) || 0), 1)
 }, [rows])
 ...
 return (
  ...
  key={`${row.price}-${idx}`}
  className={clsx(
  'relative grid grid-cols-3 gap-1 px-2 py-1 text-xs border-b border-dark-800/50 hover:bg-dark-800/40',
  ...
  isBid ? 'left-0 bg-accent-green/[0.08]' : 'right-0 bg-accent-red/[0.08]'
  )}
  style={{ width: `${pct}%` }}
  />
  <span className="relative text-dark-300 tabular-nums">{formatLot(row.volume)}</span>
  <span className="relative text-center text-dark-400 tabular-nums">{row.que_num}</span>
  <span className="relative text-right font-medium tabular-nums">{formatPrice(row.price)}</span>
  </div>
 )
 })}
 </div>
