...
import { Card, ErrorState, SkeletonLoader } from '@/components'

function stockIconUrl(code: string, fromApi?: string): string {
 return fromApi || `https://assets.stockbit.com/logos/companies/${code.toUpperCase()}.png`
}
...
export default function TrendingStocksGrid() {
   ...
   <div>
    <div className="mb-4 flex items-end justify-between">
     ...
     <div className="flex gap-2">
      <div className="h-6 w-12 bg-dark-800 rounded animate-pulse" />
      <div className="h-6 w-12 bg-dark-800 rounded animate-pulse" />
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
     {Array.from({ length: 8 }).map((_, idx) => (
      <SkeletonLoader key={idx} variant="card" height={190} />
     ))}
    </div>
   </div>
  )
 ...
 const iconUrl = stockIconUrl(code, stock.iconUrl || stock.logo || stock.company_logo)
 ...
 src={iconUrl}
 alt=""
 className="h-10 w-10 flex-shrink-0 rounded-full bg-dark-800 object-cover"
 />
 <div className="min-w-0">
 <div className="font-bold text-dark-100 text-lg leading-tight">{code}</div>
 <div className="text-xs text-dark-400 truncate">{stock.name}</div>
 </div>
 </div>
