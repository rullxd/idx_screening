...
import AccumulationPanel from '@/components/Market/AccumulationPanel'
        ...
        <div className="space-y-6">
            ...
            <Card className="p-4">
                <p className="text-sm text-dark-400 mb-3">
                    Analisis bandar & ringkasan broker buy/sell. Gunakan untuk deteksi fase akumulasi atau distribusi.
                </p>
            ...
            {isLoading ? (
                ...
                <>
                    ...
                    {/* ── BARU: Bandarscope Accumulation Panel ── */}
                    ...
                    {summary && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <SummaryCard label="Rata-rata Harga" value={`Rp ${summary.average.toFixed(0)}`} />
                            <SummaryCard
                                label="Avg Acc/Dist"
                                value={summary.accdist}
                                className={accdistColor(summary.accdist)}
                            />
                            <SummaryCard
                                label="Broker Acc/Dist"
                                value={summary.brokerAccdist}
                                className={accdistColor(summary.brokerAccdist)}
                            />
                            <SummaryCard
                                label="Pembeli"
                    ...
                    {tiers && (
                        <Card className="p-4">
                            <h4 className="text-sm font-semibold text-dark-200 mb-3">
                                📊 Konsentrasi Bandar — Timeframe
                            </h4>
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                                <TierCard label="Avg" tier={tiers.avg} />
                                <TierCard label="Avg 5 Hari" tier={tiers.avg5} />
                            ...
                            <p className="text-[10px] text-dark-600 mt-2">
                                Makin besar % → makin terkonsentrasi di sedikit broker → sinyal bandar semakin kuat
                            </p>
                        </Card>
                    )}
...
function MarketDetectorMeta({
    ...
    return (
        <div className="flex items-center justify-between text-sm text-dark-500">
            ...
            {isFetching && <span className="animate-pulse text-xs">memperbarui…</span>}
        </div>
    )
}
